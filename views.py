...
def group_write_db(request):
    if request.method == "POST":
        ...
        Posting.objects.create(user=AuthUser.objects.get(username=request.user.username),
                               group=Group.objects.get(group_id=groupID), title=request.POST.get("title", None),
...
def movie_info(request, movieID):
    ...
    return render(request, 'blog/movie_info.html', {'movie': temp, 'genres': genreTemp, 'directors': directorTemp,
                                                    'roles_and_actors': zip(roleTemp, actorTemp)})
...
def QA_create_db(request):
    if request.method == "POST":
        QA.objects.create(category=request.POST.get('category', None),
                          user=AuthUser.objects.get(username=request.user.username),
